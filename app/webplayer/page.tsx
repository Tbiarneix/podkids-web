...
export default function WebPlayer() {
  ...
  useEffect(() => {
    try {
      const raw =
        typeof window !== "undefined" ? window.localStorage.getItem(LS_KEY_ONLY_SUBS) : null;
  ...
  useEffect(() => {
    try {
      if (typeof window !== "undefined")
        window.localStorage.setItem(LS_KEY_ONLY_SUBS, onlySubs ? "1" : "0");
    } catch {}
  }, [onlySubs]);
  ...
  const toCardProps = (podcast: PodcastRow) => ({
    ...
    categories: Array.isArray(podcast.categories) ? podcast.categories.map((c) => toLabel(c)) : [],
  ...
  const allowedByAge: PodcastRow[] = useMemo(() => {
    ...
    const allowedCodes = new Set(
      activeRanges.map((r) => AGE_RANGE_CODE_MAP[r as AgeRange] ?? String(r)),
    );
        ...
        <div className="mb-4 flex items-center justify-between gap-3">
          <Suspense fallback={null}>
            <CategoryFilter
              ...
              extractCategories={(p: PodcastRow) =>
