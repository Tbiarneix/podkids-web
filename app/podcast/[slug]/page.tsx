    ...
    const load = async () => {
      try {
        const { data: list } = await supabase
          .from("podcast")
          .select(
            "id, name, author, cover_url, categories, episodes_count, description, subscription, is_subscribed",
          );
  ...
  return (
    <div className="w-full">
      <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        <div
          className={cn(
            "flex flex-col gap-6 rounded-2xl border bg-card/95 p-6 text-card-foreground shadow-sm",
          )}
        >
              ...
              <div className="mt-5">
                <button
                  ...
                  className={cn(
                    ...
                    subscribed
                      ? "bg-white text-slate-900 hover:bg-white/90 focus-visible:ring-white"
          ...
          <ul className="flex flex-col gap-4">
            {episodes.map((ep) => {
              ...
              return (
                <li
                  key={ep.id}
                  className="flex items-start gap-4 rounded-2xl border bg-card/95 p-4 text-card-foreground shadow-sm"
                >
                  <Image
                    src={epCover}
                    alt={ep.name}
